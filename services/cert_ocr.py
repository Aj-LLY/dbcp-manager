...
class CertOCRService:
    ...
    完整识别流程（5 步管道）:
    1. 检查文件存在性  ->  文件不存在则返回空结果，避免后续报错
    2. 文件转图片字节   ->  PDF 通过 PyMuPDF 渲染为 300 DPI PNG，图片直接读取二进制
    3. OCR 文字识别     ->  easyocr.Reader.readtext() 输出 (边界框, 文本, 置信度) 列表
    4. 置信度过滤       ->  仅保留置信度 > 0.2 的文本，平衡召回率和精确率
    5. 结构化字段提取   ->  正则 + 关键词策略提取 5 个目标字段

    识别的目标字段:
    - company_name: 备案单位名称（公司名称）
    - system_name: 被测评系统名称
    - cert_number: 证书编号（格式：11位数字-5位数字）
    - issue_date: 证书颁发日期（标准化为 YYYY-MM-DD）
    - level: 系统保护等级（归一化为：第X级）

    单例 + 延迟初始化设计的原因:
    - easyocr.Reader 初始化耗时数秒至数十秒（需要加载 PyTorch 模型）
    - 程序启动时不应因为可选功能阻塞主流程
    - 重复创建 Reader 会浪费内存（模型数据占用数百 MB）
    ...
    def _file_to_image(self, file_path: str) -> bytes:
        ...
        if ext == ".pdf":
            ...
            # 打开 PDF 文档（fitz.open 返回 Document 对象）
            doc = fitz.open(file_path)
            # 获取第一页（等保备案证通常是单页文件，或多页中的首页包含关键信息）
            # page 索引从 0 开始，doc[0] = 第一页
            page = doc[0]
            # 渲染页面为图片：300 DPI 提供足够的分辨率用于 OCR 识别
            # DPI 越高文字越清晰但图片越大、处理越慢；300 是经验上的平衡值
            pix = page.get_pixmap(dpi=300)
            # 将渲染结果导出为 PNG 格式的字节数据（PNG 是无损压缩，保证文字质量）
            img_bytes = pix.tobytes("png")
            # 显式关闭 PDF 文档释放文件句柄和内存
            doc.close()
            return img_bytes
    ...
    def _extract_cert_number(self, text: str) -> str:
        ...
        # 例如: "33000000000-24001"
        # {11} = 恰好 11 个数字（区域码 + 机构码），{5} = 恰好 5 个数字（序号）
    ...
    def _extract_level(self, text: str) -> str:
        ...
        # 匹配的特殊情况：
        # - "第?": "第"是可选的（部分备案证格式是"二级"而非"第二级"）
        # - "\s*": 允许空白（OCR 可能将字符识别为空格分隔）
        # - 数字范围 1-5：等保等级只有 5 个级别
    ...
    def _extract_date(self, text: str) -> str:
        ...
        Returns:
            str: YYYY-MM-DD 格式的日期字符串（月份和日期补零到两位），
                 未找到时返回空字符串。
        """
        # 策略1: 匹配中文日期格式 "YYYY年MM月DD日"（等保证书上最常见格式）
        # \s* 匹配可能的空白字符（OCR 可能将空格插入年月日之间）
        # 月份和日期允许 1-2 位数字（如 "1月" 和 "12月"）
        m = re.search(r"(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日", text)
        if m:
            y, mo, d = m.groups()
            # 补零：将 "1" 格式化为 "01"，int 转换确保处理 "01" 这样的字符串
